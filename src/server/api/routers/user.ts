  ...
  getProfile: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const stats = {
        ...
        totalLikes: user.promptCards.reduce(
          (sum, card) => sum + card.likesCount,
          0,
        ),
        totalForks: user.promptCards.reduce(
          (sum, card) => sum + card.forkCount,
          0,
        ),
      };
  ...
  getFavoriteCards: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const validLikes = likes.filter(
        (like) => like.promptCard && !like.promptCard.isDeleted,
      );
